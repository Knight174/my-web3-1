...
"三明治攻击"（Sandwich Attack）属于针对区块链交易的一种前端运行策略（Front-Running Strategy），特别是在去中心化金融服务（DeFi）中的交易。其主要目标是通过观察和利用未确认的交易，来在可以预见价格变动的情况下，通过价格操纵获取利益。

三明治攻击（Sandwich Attack）的运作过程一般如下：

- 攻击者在交易池中观察到一个对他有利，但还未被网络确认的交易。例如，某个用户准备在 Uniswap 交易所购买大量的某个币种。
- 攻击者首先创建一个以更高手续费（因此更优先被矿工采纳）的交易，购买相同的币种。由于这个币种的需求增加，价格会上升。
- 当攻击者的这个交易被网络确认后，之前被观察到的用户的购买交易也被确认。但由于价格上升，用户的交易购买的币种数量会比原来少。
- 攻击者再次以更高的手续费，卖出他之前购买的币种，价格随着供应量的增加而下降。
- 通过这种方式，攻击者的买入价格低于卖出价格，从而获取利润。而之前的用户却因为交易价格的变动，得到了比他原本预计的更少的币种。

为了防止三明治攻击，用户和开发者通常会设定一定的滑点（即允许的价格变动范围），或者通过其他一些防止前端运行的方法来进行防御。

## 抢跑

"抢跑"（Front-Running）是一种在传统金融市场和区块链交易中都存在的不公平交易行为。当一方能够在另一方交易之前获得交易信息，并据此优先进行交易以获取利益，这种行为就是抢跑。

以区块链交易为例，由于所有的区块链交易都是公开且透明的，一些机器人或自动化脚本可能会扫描即将被矿工打包的交易，通过分析即将发生的交易形势，可以提前进行交易并从中获利。

例如，如果一个大额购买订单即将被执行，可能会导致代币价格上涨，那么抢跑攻击者就可以在大额交易被打包到区块之前，先以更高的手续费发送一个购买订单，由于手续费更高，攻击者的交易更可能被矿工优先打包。之后价格上涨，攻击者再卖出代币，从而获得利润。

在以太坊等智能合约平台上，抢跑的问题更严重，因为通过智能合约可以进行更为复杂的操作，常见的抢跑方式包括交易插队和三明治攻击。

为了防止抢跑，开发者可以在智能合约上设置一些防御机制，如引入随机性，使得抢跑攻击者无法确定交易的结果；还可以通过设计交易计费公平的系统，引导用户选择合理的手续费，避免抢跑攻击。另外，一些 Layer 2 解决方案如 zkRollups，Optimistic Rollups 等也能一定程度上解决抢跑问题。
...
空投发放的加密货币可以根据项目的需求，有不同的要求和限制。例如，有些项目可能要求接受空投的用户在社交媒体上关注或分享相关内容，或者要求用户已经持有一定数量的其他特定加密货币。有些空投甚至会要求用户进行一些更为复杂的任务，例如参与产品的测试。

## Dex(Decentralized Exchange, DEX)

DEX Swap 是指在去中心化交易所(Decentralized Exchange, DEX)上进行的代币交换操作。去中心化交易所是一种允许在区块链网络上直接进行点对点交易的平台。
